...
function UserProvider({ config, ...props }) {
  ...
  const localdriveRef = useRef(
    new Localdrive(downloadsFolder() + Pear.config.storage)
  );
  ...
  async function getFiles() {
    ...
    const mirror = hyperdriveRef.current.mirror(localdriveRef.current, {
      prefix: "/files",
    });
    mirror.done();

    console.log("mirror.count:", mirror.count);
